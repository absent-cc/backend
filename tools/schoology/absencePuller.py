...
class VariableSchoologyWriter:
    ...
    def updateDatabase(
        self,
        filteredAbsences: dict[structs.SchoolName, list[str]],
        date: datetime,
        db: Session = accounts.getDBSession,
    ):
        for school, absences in filteredAbsences.items():
            ...
            for absence in absences:
                if not self.sc.addAbsence(absence):
                    print(
                        f"Absences already exist for {date}. Probs some error in your code."
                    )
                    continue
    ...
    def writeRawToFile(
        self,
        raw: dict[structs.SchoolName, list[str]],
        date: datetime,
        rawFileBasePath: str = f"{relative_path}/data/rawFeed",
    ):
