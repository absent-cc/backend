...
class UserProfilePopulate:
    ...
    def populateDB(self) -> None:
        for user in self.data:
            print(user)
            userEntry = crud.addUser(
                self.db, schemas.UserCreate(**user.profile.__dict__)
            )
            # print(type(userEntry))
            print(userEntry)
            userObject = schemas.UserReturn(**userEntry.__dict__)
            crud.updateSchedule(
                self.db, userObject, schemas.Schedule(**user.schedule.__dict__)
            )
            crud.updateUserSettings(
                self.db,
                schemas.UserSettings(**user.settings.__dict__),
                uid=userObject.uid,
            )
