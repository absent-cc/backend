...
def verifyCredentials(
    creds: str = Security(credsHeader), db=Depends(getDBSession)
...
def validateGoogleToken(token) -> dict:
    ...
    BACKEND_ID = (
        "349911558418-9d07ptkk7pg7aqq58qkj5tshi8bq9s5v.apps.googleusercontent.com"
    )
    ...
    CLIENT_IDs = [BACKEND_ID, EXPO_ID, IOS_ID, ANDROID_ID, DEV_IOS_ID, DEV_ANDROID_ID]
    ...
    try:
        idInfo = id_token.verify_token(
            token.token, requests.Request(), audience=CLIENT_IDs
        )
...
def generateToken(clientID: str) -> str:
    ...
    webtoken = jwt.encode(payload=payload, key=key, algorithm="RS256")
